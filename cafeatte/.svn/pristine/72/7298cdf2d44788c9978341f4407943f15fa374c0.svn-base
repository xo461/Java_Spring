...
public class UserCafeDAO {
	...
	public int delete(int no, String id) throws Exception{
		...
		Connection con = DBUtil.getConnection();
		String sql = " delete from userCafe where no=? ";
		if (id.equals("admin")) {
			sql+="";
		}else {
			sql+= " and id=? "; 
		...
		PreparedStatement pstmt = con.prepareStatement(sql);
		if (id.equals("admin")) { //string은 ==로비교하면 안되고 .equals로 비교해야 한다.
			pstmt.setInt(1, no);
		}else {
			pstmt.setInt(1, no);
	...
	public int userLikeList(int no, String id) throws Exception{
		...
		PreparedStatement pstmt = con.prepareStatement(sql);
		pstmt.setInt(1, no);
		pstmt.setString(2, id);//jsp에서 받아온 no와 id를 넣어서 같은 no와 id에 해당하는 데이터를 db에서 불러온다. -> null이 아니면 좋아요를 한 상태라는 뜻. 
		ResultSet rs = pstmt.executeQuery();
		if (rs!=null && rs.next()) {
			DBUtil.close(con, pstmt, rs);
			return 1;//좋아요를 한 상태 (userLike테이블 안에 게시글넘버와 좋아요한사람id가 들어있다는의미)
		}
		DBUtil.close(con, pstmt, rs);
