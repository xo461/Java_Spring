			...
			//
				...
				try {
					...
					if(id!="") {//로그인되어있으면
						...
						request.setAttribute("result", Execute.service(userLikeService, no, id, 0));//마지막 0은 vr. 그냥 view 클릭시에는 vr=0으로 넘김 / 로그인되어있는상태로 하트 클릭시 vr=1로 넘김
						System.out.println(request.getAttribute("result"));
						//"result" : dao -> service -> controller로 받은 값. (0이면 해당 아이디로 좋아요한적없음, 1이면 있음)
					}else {
