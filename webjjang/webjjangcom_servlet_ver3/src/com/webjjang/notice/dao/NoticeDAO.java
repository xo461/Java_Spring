...
//		2) 순서번호붙이기
		sql = " select rownum rnum, no, title, startDate, endDate, writeDate, updateDate, hit from ("+sql+") ";
...
//		3) 페이지에 맞는 데이터 찾아오기
		...
		//위의 첫번째 물음표에 대한 값은 StartRow에서 가져와서 저장
		pstmt.setInt(1, pageObject.getStartRow());
		//위의 두번째 물음표에 대한 값은 StartRow에서 가져와서 저장
	...
	// service.NoticeListService -> dao.NoticeDAO
		// 1-1. 공지사항 리스트 데이터 가져오기 - 전체 데이터의 갯수를 가져오는 메서드
		//  데이터 가져오기가 실패하면 출력하러 갈 수가 없다. 그런 경우에는 예외처리를 반드시 해야 하므로
		//  여기서는 예외처리하지 않고 throw 시키는 것으로 할 수 있다.
		...
		public int getTotalRow() throws Exception{
			...
			if(rs != null && rs.next()) {
				totalRow = rs.getInt("cnt"); //select에 있는걸 써야 함.
