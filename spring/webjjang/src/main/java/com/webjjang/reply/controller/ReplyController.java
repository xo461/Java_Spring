...
public class ReplyController {
	...
	@Autowired
	@Qualifier("rs")
	// ReplyserviceImpl이 Replyservice상속받으므로 replycontroller에서 replyservice 변수선언시
	// bean이 replyservice와 replyserviceimpl 두개 가 된다 -> 같은 타입의 빈이 두 개 이상이 존재하는 경우에
	// 스프링이 어떤 빈을 주입해야 할지 알 수 없어서 스프링 컨테이너를 초기화하는 과정에서 예외를 발생시킨다.
	// 이 경우 @Qualifier을 @Autowired와 함께 사용하여 정확히 어떤 bean을 사용할지 지정하여 특정 의존 객체를 주입할 수
	// 있도록 한다. (해당 서비스에도 @Qualifier("같은이름")어노테이션 써줘야함)
	...
	// responseEntity: 상태코드(필수), 응답메시지를 리턴해준다.
	// json, xml주고받을때: @RequestBody 어노테이션을 이용하면 HTTP 요청 몸체를 자바 객체로 전달받을 수 있다. 비슷하게
	...
	public ResponseEntity<String> write(@RequestBody ReplyDTO dto) {
		...
		// 삼항연산자
		return insertCount == 1 ? new ResponseEntity<>("Your comment is generated.", HttpStatus.OK)
				: new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	}

	// 3. 댓글 수정 처리
	// 댓글번호는 urlpath에서 받고 나머지는 dto로 받는다.
	@RequestMapping(value = "/{rno}", consumes = "application/json", produces = "application/text; charset=utf8", method = RequestMethod.PATCH)
	public ResponseEntity<String> update(@RequestBody ReplyDTO dto, @PathVariable int rno) {
		...
		if (result > 0)// 성공적으로 수정되어 1이 리턴되면
			return new ResponseEntity<>("The comment is updated.", HttpStatus.OK);
		else //비번틀리면 0리턴됨
			return new ResponseEntity<>("Please check your password.", HttpStatus.NOT_ACCEPTABLE);
